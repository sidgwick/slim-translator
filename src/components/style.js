{
  "slim_translate_root": {
    "padding": "0",
    "margin": "0"
  },
  "slim_translate_button_area": {
    "position": "absolute",
    "backgroundColor": "rgb(245, 245, 245)",
    "boxSizing": "content-box",
    "cursor": "pointer",
    "height": "19px",
    "width": "19px",
    "zIndex": "2147483647",
    "borderWidth": "1px",
    "borderStyle": "solid",
    "borderColor": "rgb(220, 220, 220)",
    "borderImage": "initial",
    "borderRadius": "5px",
    "padding": "3px"
  },
  "slim_translate_button_area_button": {
    "backgroundImage": "url(\"data:image/png;base64,\")",
    "backgroundSize": "19px",
    "height": "19px",
    "width": "19px"
  },
  "slim_translate_button_area_button_hover": {
    "cursor": "pointer",
    "backgroundColor": "#ffffff !important"
  },
  "slim_translate_button_area_button_active": {
    "backgroundColor": "#ffffff !important"
  },
  "slim_translate_result_area": {
    "boxShadow": "rgba(0, 0, 0, 0.2) 0px 1px 3px",
    "backgroundColor": "rgb(255, 255, 255)",
    "position": "absolute",
    "zIndex": "1201 !important",
    "borderWidth": "1px",
    "borderStyle": "solid",
    "borderImage": "initial",
    "borderColor": "rgb(187, 187, 187) rgb(187, 187, 187) rgb(168, 168, 168)",
    "padding": "16px",
    "minWidth": "200px",
    "maxWidth": "400px",
    "color": "#252525",
    "fontFamily": "'Roboto',arial,sans-serif",
    "fontSize": "14px",
    "margin": "0"
  },
  "slim_translate_result_area__gtx_language": {
    "color": "#636363",
    "fontSize": "11px",
    "margin": "0",
    "padding": "5px 0"
  },
  "slim_translate_result_area__gtx_target_language": {
    "marginTop": "3px"
  },
  "slim_translate_result_area__gtx_lang_selector": {
    "backgroundColor": "#f5f5f5",
    "backgroundImage": "-webkit-linear-gradient(top, #f5f5f5, #f1f1f1)",
    "border": "1px solid #dcdcdc",
    "color": "#444",
    "height": "25px",
    "outline": "none"
  },
  "slim_translate_result_area__gtx_body": {
    "display": "inline",
    "fontSize": "16px",
    "margin": "5px auto",
    "padding": "5px 0"
  },
  "slim_translate_result_area__gtx_audio_button": {
    "border": "1px solid transparent",
    "borderRadius": "0",
    "cursor": "default",
    "display": "inline-block",
    "fontSize": "11px",
    "fontWeight": "bold",
    "height": "21px",
    "lineHeight": "21px",
    "marginRight": "1px",
    "outline": "0px",
    "padding": "0",
    "textAlign": "center",
    "whiteSpace": "nowrap"
  },
  "slim_translate_result_area__gtx_audio_button__gtx_button_img": {
    "backgroundImage": "url(\"data:image/png;base64,\")",
    "backgroundSize": "91%",
    "opacity": ".45",
    "height": "21px",
    "width": "21px",
    "verticalAlign": "middle",
    "display": "inline-block"
  },
  "slim_translate_result_area__gtx_link": {
    "color": "#a2a2a2",
    "paddingTop": "16px",
    "display": "inline",
    "fontSize": "11px",
    "textDecoration": "none"
  },
  "slim_translate_result_area__gtx_link_hover": {
    "textDecoration": "underline"
  },
  "slim_translate_result_area__gtx_link_left": {
    "float": "left"
  },
  "slim_translate_result_area__gtx_link_right": {
    "float": "right"
  }
}